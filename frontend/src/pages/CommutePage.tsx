...
import { useState, useCallback, useRef, useId, ReactNode, ReactElement, FormEvent } from 'react';
...
interface ResultCardProps {
  result: CommuteResultData;
  onSave: () => void;
  saving: boolean;
}

// ── Emission result card ──────────────────────────────────────────────────────
function ResultCard({ result, onSave, saving }: ResultCardProps): ReactElement {
  const { distanceKm, durationMinutes, kgCO2e, mode, comparisonDriving } = result;

  const savingsKg =
    comparisonDriving !== null &&
    comparisonDriving !== undefined &&
    kgCO2e !== null &&
    kgCO2e !== undefined
  ...
  return (
    <div
      className="card"
      role="region"
      aria-label="Commute emission calculation results"
      style={{
    ...
    >
      ...
      {/* Savings banner */}
      {savingsKg !== null && savingsKg !== undefined && parseFloat(savingsKg) > 0 && (
        <div
          style={{
...
export default function CommutePage() {
  ...
  const [result, setResult]         = useState<CommuteResultData | null>(null);
  ...
  const handleCalculate = useCallback(
    async (e: FormEvent) => {
      ...
      if (!user) {
        setError('You must be logged in to calculate commute emissions.');
        return;
      }
