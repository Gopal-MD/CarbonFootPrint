...
// ── Scan result display ──────────────────────────────────────────────────────
function ScanResultCard({ data }: { data: ScanResultData }) {
  const { kWhExtracted, billMonth, provider, kgCO2e, rawText } = data;

  const items = [
    { label: 'Energy Consumed',    value: kWhExtracted !== null && kWhExtracted !== undefined ? `${kWhExtracted} kWh` : 'Not detected', icon: '⚡' },
    { label: 'CO₂ Emitted',        value: kgCO2e !== null && kgCO2e !== undefined       ? `${kgCO2e} kg`        : 'N/A',          icon: '🌫️' },
...
export default function ScanPage() {
  ...
  const validateAndSet = useCallback((selectedFile: File | undefined) => {
    setError('');
    setResult(null);
  ...
  const handleFileChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => validateAndSet(e.target.files?.[0]),
    [validateAndSet]
  );
  ...
  const handleScan = useCallback(async () => {
    ...
    if (!user) {
      setError('You must be logged in to scan a bill.');
      return;
    }
      ...
      <div className="card" style={{ marginBottom: 'var(--space-6)' }}>
        ...
        {!file ? (
          ...
          >
            ...
            <p id="file-requirements" style={{ fontSize: 'var(--text-sm)', color: 'var(--text-muted)' }}>
