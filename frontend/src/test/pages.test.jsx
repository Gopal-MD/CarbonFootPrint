...
describe('CommutePage', () => {
  ...
  beforeEach(async () => {
    ...
    const mod = await import('@pages/CommutePage.jsx');
  ...
  it('shows validation error when fields are empty and form submitted', async () => {
    renderWithRouter(<CommutePage />);
    const submitBtn = screen.getByRole('button', { name: /Calculate Emissions/i });
  ...
  it('renders emission factors info section', () => {
    renderWithRouter(<CommutePage />);
    expect(screen.getByText(/Emission Factors/i)).toBeInTheDocument();
    expect(screen.getByText(/0.210 kg CO₂e \/ km/i)).toBeInTheDocument();
  });
  ...
  it('calls fetch on form submit with valid data', async () => {
    mockFetch.mockResolvedValueOnce({
      ...
      json: async () => ({
        success: true,
        data: { distanceKm: 15.2, durationMin: 22, kgCO2e: 3.19, mode: 'DRIVING', comparisonDriving: 3.19 },
      }),
    });

    renderWithRouter(<CommutePage />);
    await userEvent.type(screen.getByLabelText(/Origin Address/i), '123 Main St');
    await userEvent.type(screen.getByLabelText(/Destination Address/i), '456 Oak Ave');
    ...
    await waitFor(() => {
      expect(mockFetch).toHaveBeenCalledWith(
        expect.stringContaining('/api/commute'),
        expect.objectContaining({ method: 'POST' })
      );
    });
  });

  it('shows result card after successful API response', async () => {
    mockFetch.mockResolvedValueOnce({
      ok: true,
      json: async () => ({
        success: true,
    ...
    await waitFor(() => {
      expect(screen.getByRole('region', { name: /Commute calculation result/i })).toBeInTheDocument();
  ...
  it('shows error message on API failure', async () => {
    mockFetch.mockResolvedValueOnce({
      ok: false,
      json: async () => ({ success: false, message: 'Google Maps API error' }),
    });
    ...
    await waitFor(() => {
      expect(screen.getByRole('alert')).toHaveTextContent(/Google Maps API error/i);
    });
  });

  it('has accessible section landmark with aria-label', () => {
    renderWithRouter(<CommutePage />);
    // The form has aria-label but jsdom may not expose form role from <form>
    expect(screen.getByLabelText(/commute calculator form/i)).toBeInTheDocument();
  });
});
...
describe('ScanPage', () => {
  ...
  beforeEach(async () => {
    vi.resetAllMocks();
    mockGetIdToken.mockResolvedValue('mock-id-token');
    const mod = await import('@pages/ScanPage.jsx');
  ...
  it('renders how it works section', () => {
    ...
    expect(screen.getByText(/Gemini Vision AI reads and extracts/i)).toBeInTheDocument();
  });
...
describe('InsightsPage', () => {
  ...
  beforeEach(async () => {
    vi.resetAllMocks();
    mockGetIdToken.mockResolvedValue('mock-id-token');
    const mod = await import('@pages/InsightsPage.jsx');
  ...
  it('renders Generate AI Insights button', () => {
    renderWithRouter(<InsightsPage />);
    // aria-label is the full accessibility label; match by visible text content
