...
// ── API Routes ────────────────────────────────────────────────────────────────
router.use('/api/commute', (req, _res, next) => {
  logger.http(`${req.method} /api/commute`);
  next();
}, commuteRouter);

router.use('/api/scan', (req, _res, next) => {
  logger.http(`${req.method} /api/scan`);
  next();
}, scanRouter);

router.use('/api/insights', (req, _res, next) => {
  logger.http(`${req.method} /api/insights`);
  next();
}, insightsRouter);

router.use('/api/emissions', (req, _res, next) => {
  logger.http(`${req.method} /api/emissions`);
  next();
}, emissionsRouter);

router.use('/api/auth', (req, _res, next) => {
